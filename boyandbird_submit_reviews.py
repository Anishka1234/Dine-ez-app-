...
def open_reviews():
    subprocess.Popen(['python', 'boyandbird_submit_reviews.py'])
...
def open_submit_reviews():
    ...
    script_path = 'boyandbird_submit_reviews.py'
...
def submit_review():
    ...
    if review_text != '' and username:
        ...
        try:
            cursor.execute('''
                INSERT INTO boyandburger_reviews (username, user_review) VALUES (?, ?)
...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS boyandburger_reviews (
