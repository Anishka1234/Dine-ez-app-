...
def retrieve_restaurant_data():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("SELECT image_path, website FROM healthy LIMIT 3 OFFSET 3")
        data = c.fetchall()
        return data
...
def get_restaurant_names():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM healthy LIMIT 3 OFFSET 3")
...
def get_restaurant_locations():
    conn = sqlite3.connect('restaurants.db')
    cursor = conn.cursor()
    cursor.execute("SELECT location FROM healthy LIMIT 3 OFFSET 3")
    data = cursor.fetchall()
    conn.close()
    return [item[0] for item in data]
...
menu_restaurant_1_reviews.add_command(label="ADD REVIEW", command=lambda: open_window("forest_submit_reviews.py"))
menu_restaurant_1_reviews.add_command(label="VIEW REVIEWS",
                                      command=lambda: open_window("forest_display_reviews.py"))
...
menu_restaurant_2_reviews.add_command(label="ADD REVIEW",
                                      command=lambda: open_window("mekong_baby_submit_reviews.py"))
menu_restaurant_2_reviews.add_command(label="VIEW REVIEW",
                                      command=lambda: open_window("mekong_baby_display_reviews.py"))
...
menu_restaurant_3_reviews.add_command(label="ADD REVIEW",
                                      command=lambda: open_window("khu_khu_eatery_submit_reviews.py"))
menu_restaurant_3_reviews.add_command(label="VIEW REVIEW",
                                      command=lambda: open_window("khu_khu_eatery_display_reviews.py"))
...
# Text to indicate Healthy options on the user interface
canvas.create_text(621.0, 220.0, anchor="nw", text="○ HEALTHY", fill="#000000", font=("Inter Light", 25))
canvas.create_text(80.0, 788.0, anchor="nw", text="◉ Healthy", fill="#000000", font=("Inter ExtraLightItalic", 12))
canvas.create_text(546.0, 788.0, anchor="nw", text="◉ Healthy", fill="#000000", font=("Inter ExtraLightItalic", 12))
canvas.create_text(1012.0, 788.0, anchor="nw", text="◉ Healthy", fill="#000000", font=("Inter ExtraLightItalic", 12))
...
restaurant_name_text_2 = canvas.create_text(620.0, 713.0, anchor="nw", text="NAME OF RESTAURANT",
                                            fill="#000000", font=("Inter Light", 25))
...
restaurant_location_text_2 = canvas.create_text(570.0, 757.0, anchor="nw", text="location",
                                                fill="#000000", font=("Inter Light", 17))
