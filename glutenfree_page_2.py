...
def retrieve_restaurant_data():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("SELECT image_path, website FROM gluten_free LIMIT 3 OFFSET 3")
        data = c.fetchall()
        return data
...
def get_restaurant_names():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM gluten_free LIMIT 3 OFFSET 3")
...
def get_restaurant_locations():
    conn = sqlite3.connect('restaurants.db')
    cursor = conn.cursor()
    cursor.execute("SELECT location FROM gluten_free LIMIT 3 OFFSET 3")
    data = cursor.fetchall()
    conn.close()
    return [item[0] for item in data]
...
menu_restaurant_1_reviews.add_command(label="ADD REVIEW", command=lambda: open_window("sideart_submit_reviews.py"))
menu_restaurant_1_reviews.add_command(label="VIEW REVIEWS",
                                      command=lambda: open_window("sideart_display_reviews.py"))
...
menu_restaurant_2_reviews.add_command(label="ADD REVIEW",
                                      command=lambda: open_window("baduzzi_submit_reviews.py"))
menu_restaurant_2_reviews.add_command(label="VIEW REVIEW",
                                      command=lambda: open_window("baduzzi_display_reviews.py"))
...
menu_restaurant_3_reviews.add_command(label="ADD REVIEW",
                                      command=lambda: open_window("onetreegrill_submit_reviews.py"))
menu_restaurant_3_reviews.add_command(label="VIEW REVIEW",
                                      command=lambda: open_window("onetreegrill_display_reviews.py"))
review_button_3.bind("<Button-1>", lambda event: show_menu(event, menu_restaurant_2_reviews))


# Navigation buttons and text placed on canvas
# 'Backward' button that navigates and allows the user to be directed to the previous page of restaurants
button_image_back_arrow = PhotoImage(file=relative_to_assets("backarrow.png"))
button_back_arrow = Button(image=button_image_back_arrow, borderwidth=0, highlightthickness=0,
                           command=lambda: open_gluten_free_page_1(), relief="flat")
button_back_arrow.place(x=18.0, y=823.0, width=82.0, height=66.0)


# 'Vegan' button that navigates to the first page of vegan options
button_image_vegan = PhotoImage(
    file=relative_to_assets("vegan.png"))
...
button_image_gluten_free = PhotoImage(
    file=relative_to_assets("gluten_free.png"))
...
button_image_keto = PhotoImage(file=relative_to_assets("keto.png"))
...
button_image_healthy = PhotoImage(
    file=relative_to_assets("healthy.png"))
...
account_button_image = PhotoImage(
...
image_blank_1 = PhotoImage(
    file=relative_to_assets("blank_1.png"))
...
# Text to indicate Gluten-free options on the user interface
canvas.create_text(621.0, 220.0, anchor="nw", text="○ GLUTEN-FREE", fill="#000000", font=("Inter Light", 25))
canvas.create_text(80.0, 788.0, anchor="nw", text="◉ Gluten-free", fill="#000000", font=("Inter ExtraLightItalic", 12))
canvas.create_text(546.0, 788.0, anchor="nw", text="◉ Gluten-free", fill="#000000", font=("Inter ExtraLightItalic", 12))
canvas.create_text(1012.0, 788.0, anchor="nw", text="◉ Gluten-free", fill="#000000", font=("Inter ExtraLightItalic", 12))
...
restaurant_name_text_2 = canvas.create_text(640.0, 713.0, anchor="nw", text="NAME OF RESTAURANT",
                                            fill="#000000", font=("Inter Light", 25))
restaurant_name_text_3 = canvas.create_text(1100.0, 713.0, anchor="nw", text="NAME OF RESTAURANT",
                                            fill="#000000", font=("Inter Light", 25))
...
restaurant_location_text_2 = canvas.create_text(550.0, 757.0, anchor="nw", text="location",
                                                fill="#000000", font=("Inter Light", 17))
