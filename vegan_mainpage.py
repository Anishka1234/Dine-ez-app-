...
def retrieve_restaurant_data():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("SELECT image_path, website FROM vegan LIMIT 3")
        data = c.fetchall()
        return data
...
def get_restaurant_names():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM vegan LIMIT 3")
...
def get_restaurant_locations():
    conn = sqlite3.connect('restaurants.db')
    cursor = conn.cursor()
    cursor.execute("SELECT location FROM vegan LIMIT 3")
    data = cursor.fetchall()
    conn.close()
    return [item[0] for item in data]
...
def open_vegan_page_2():
    window.destroy()
    subprocess.Popen(['python', 'vegan_page_2.py'])
...
menu_restaurant_1_reviews.add_command(label="VIEW REVIEWS", command=lambda: open_window("forest_display_reviews.py"))
...
menu_restaurant_2_reviews.add_command(label="ADD REVIEW", command=lambda: open_window("atelier_submit_reviews.py"))
menu_restaurant_2_reviews.add_command(label="VIEW REVIEW", command=lambda: open_window("atelier_display_reviews.py"))
...
menu_restaurant_3_reviews.add_command(label="ADD REVIEW",
                                      command=lambda: open_window("bali_nights_submit_reviews.py"))
menu_restaurant_3_reviews.add_command(label="VIEW REVIEW",
                                      command=lambda: open_window("bali_nights_display_reviews.py"))
...
button_forward = Button(image=button_image_forward, borderwidth=0, highlightthickness=0,
                        command=lambda: open_vegan_page_2(), relief="flat")
...
# Text indicating Vegan options on the user interface
