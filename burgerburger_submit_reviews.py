...
def open_reviews():
    subprocess.Popen(['python', 'burgerburger_submit_reviews.py'])
...
def open_submit_reviews():
    ...
    script_path = 'burgerburger_submit_reviews.py'
...
def submit_review():
    ...
    if review_text != '' and username:
        ...
        try:
            cursor.execute('''
                INSERT INTO burger_burger_reviews (username, user_review) VALUES (?, ?)
...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS burger_burger_reviews (
