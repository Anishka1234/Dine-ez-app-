...
def open_reviews():
    subprocess.Popen(['python', 'bali-nights_submit_reviews.py'])
...
def open_submit_reviews():
    ...
    script_path = 'bali-nights_submit_reviews.py'
...
def submit_review():
    ...
    if review_text != '' and username:
        ...
        try:
            cursor.execute('''
                INSERT INTO bali_nights_reviews (username, user_review) VALUES (?, ?)
...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS bali_nights_reviews (
