...
def open_reviews():
    subprocess.Popen(['python', 'khu_khu_eatery_submit_reviews.py'])
...
def open_submit_reviews():
    ...
    script_path = 'khu_khu_eatery_submit_reviews.py'
...
def submit_review():
    ...
    if review_text != '' and username:
        ...
        try:
            cursor.execute('''
                INSERT INTO khukhu_eatery_reviews (username, user_review) VALUES (?, ?)
...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS khukhu_eatery_reviews (
