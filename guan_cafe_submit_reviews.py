...
def submit_review():
    ...
    if review_text != '' and username:
        ...
        try:
            cursor.execute('''
                INSERT INTO guan_cafe_reviews (username, user_review) VALUES (?, ?)
...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS guan_cafe_reviews (
