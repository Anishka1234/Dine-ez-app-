...
def open_reviews():
    subprocess.Popen(['python', 'dosa_plaza_submit_reviews.py'])
...
def open_submit_reviews():
    ...
    script_path = 'dosa_plaza_submit_reviews.py'
...
# Function used to handle review submission
def submit_review():
    ...
    if review_text != '' and username:
        ...
        try:
            cursor.execute('''
                INSERT INTO dosa_plaza_reviews (username, user_review) VALUES (?, ?)
            ''', (username, review_with_quotes))
            conn.commit()
        finally:
            conn.close()
            user_review.delete(0, 'end')  # Clear entry after submission
...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS dosa_plaza_reviews (
