...
def open_reviews():
    subprocess.Popen(['python', 'onetreegrill_submit_reviews.py'])
...
def open_submit_reviews():
    ...
    script_path = 'onetreegrill_submit_reviews.py'
...
def submit_review():
    ...
    if review_text != '' and username:
        ...
        try:
            cursor.execute('''
                INSERT INTO onetreegrill_reviews (username, user_review) VALUES (?, ?)
...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS onetreegrill_reviews (
