...
def retrieve_restaurant_data():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("SELECT image_path, website FROM gluten_free LIMIT 3")
        data = c.fetchall()
        return data
...
def get_restaurant_names():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM gluten_free LIMIT 3")
...
def get_restaurant_locations():
    conn = sqlite3.connect('restaurants.db')
    cursor = conn.cursor()
    cursor.execute("SELECT location FROM gluten_free LIMIT 3")
    data = cursor.fetchall()
    conn.close()
    return [item[0] for item in data]
...
def open_gluten_free_page_2():
    window.destroy()
    subprocess.Popen(['python', 'glutenfree_page_2.py'])
...
menu_restaurant_1_reviews.add_command(label="ADD REVIEW", command=lambda: open_window("melba_vulcan_submit_reviews.py"))
menu_restaurant_1_reviews.add_command(label="VIEW REVIEWS", command=lambda: open_window("melba_vulcan_display_reviews.py"))
...
menu_restaurant_2_reviews.add_command(label="ADD REVIEW", command=lambda: open_window("the_grove_submit_reviews.py"))
menu_restaurant_2_reviews.add_command(label="VIEW REVIEW", command=lambda: open_window("the_grove_display_reviews.py"))
...
menu_restaurant_3_reviews.add_command(label="ADD REVIEW", command=lambda: open_window("culprit_submit_reviews.py"))
menu_restaurant_3_reviews.add_command(label="VIEW REVIEW", command=lambda: open_window("culprit_display_reviews.py"))
...
button_forward = Button(image=button_image_forward, borderwidth=0, highlightthickness=0,
                        command=lambda: open_gluten_free_page_2(), relief="flat")
...
restaurant_name_text_1 = canvas.create_text(170.0, 713.0, anchor="nw", text="NAME OF RESTAURANT",
                                            fill="#000000", font=("Inter Light", 25))
restaurant_name_text_2 = canvas.create_text(665.0, 713.0, anchor="nw", text="NAME OF RESTAURANT",
                                            fill="#000000", font=("Inter Light", 25))
...
restaurant_location_text_2 = canvas.create_text(600.0, 757.0, anchor="nw", text="location",
                                                fill="#000000", font=("Inter Light", 17))
restaurant_location_text_3 = canvas.create_text(1040.0, 757.0, anchor="nw", text="location",
                                                fill="#000000", font=("Inter Light", 17))
