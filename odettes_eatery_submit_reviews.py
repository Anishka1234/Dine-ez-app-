...
def open_reviews():
    subprocess.Popen(['python', 'odettes-eatery_submit_reviews.py'])
...
def open_submit_reviews():
    ...
    script_path = 'odettes-eatery_submit_reviews.py'
...
def submit_review():
    ...
    if review_text != '' and username:
        ...
        try:
            cursor.execute('''
                INSERT INTO odettes_eatery_reviews (username, user_review) VALUES (?, ?)
...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS odettes_eatery_reviews (
