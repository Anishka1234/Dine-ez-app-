...
def open_reviews():
    subprocess.Popen(['python', 'keto_eats_submit_reviews.py'])
...
def open_submit_reviews():
    ...
    script_path = 'keto_eats_submit_reviews.py'
...
def submit_review():
    ...
    if review_text != '' and username:
        ...
        try:
            cursor.execute('''
                INSERT INTO keto_eats_reviews (username, user_review) VALUES (?, ?)
...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS keto_eats_reviews (
